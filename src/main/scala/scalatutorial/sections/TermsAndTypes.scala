...
object TermsAndTypes extends ScalaTutorialSection {
   ...
   *
   * The contents is based on the MOOCS
   * [[https://www.coursera.org/learn/progfun1/home Functional Programming Principles in Scala]] and
   * [[https://www.coursera.org/learn/progfun2/home Functional Program Design in Scala]].
   *
   * The target audience is people who already have ''some'' experience of programming and who are
   * familiar with the JVM.
   *
   ...
   *   - ways to ''abstract'' expressions, which introduce a name for an expression by which it can
   *     then be referred to.
   *
   ...
   *
   * More complex expressions can be expressed by ''combining'' simpler expressions using
   * ''operators''. They can therefore express more complex computations:
   *
   ...
   * =Method Calls=
   *
   * Another way to make complex expressions out of simpler expressions is to call ''methods'' on
   * expressions:
   *
   ...
   *
   * In the below examples, the `abs` method returns the absolute value of a number, and the
   ...
   * =Values and Types=
   *
   * Expressions have a ''value'' and a ''type''. The evaluation model defines how to get a value
   * out of an expression. Types classify values.
   *
